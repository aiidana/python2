...
def ref_input_grid():
    ...
    if r==c and r!=0 and c!=0:
        ...
        Button(right_frame,text='Submit',bg="black", fg="white",font=('ariel', 15),width=5,command=ref_input_cmd).grid(row=r+1,column=c-1,columnspan=c,pady=10)
...
def ref_input_cmd():
    ...
    for i in dic:
        for j in i:
            sub_ref.append(j.get())
        ref_matrix.append(sub_ref)
        sub_ref=[]
    ref_result=row_echelon_form(ref_matrix)
    Label(result_frame, text='RESULT',font=('Ariel',15))
    Label(result_frame,text=ref_result,font=('ariel',15)).grid(row=50,column=0,columnspan=1,pady=10)
    result_frame.grid(row=7,column=300,padx=100)
...
Label(main_frame, text="""""", bg="black").grid(row=6,columnspan=2,padx=80,pady=2,sticky=EW)
Label(main_frame, text="""""", bg="black").grid(row=0,columnspan=2,padx=80,pady=2,sticky=EW)
