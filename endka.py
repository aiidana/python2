...
def ref_input_row_column():
    frame_destroy()

    global row_ref, col_ref
    Label(middle_frame, text="Enter number of rows of the matrix", bg='black', fg='white', borderwidth=3, relief="groove", font=('ariel', 10), width=35).grid(row=4, column=8, columnspan=5)
    Label(middle_frame, text="Enter number of columns of the matrix", bg='black', fg='white', borderwidth=3, relief="groove", font=('ariel', 10), width=35).grid(row=5, column=8, columnspan=5)

    row_ref = IntVar()
    Entry(middle_frame, width=4, textvariable=row_ref, borderwidth=3, bg='gray64', fg='black', relief="solid").grid(row=4, column=14)
    col_ref = IntVar()
    Entry(middle_frame, width=4, textvariable=col_ref, borderwidth=3, bg='gray64', fg='black', relief="solid").grid(row=5, column=14)

    Label(middle_frame, text="", bg="black").grid(row=10, column=14, padx=80, pady=2, sticky=EW)
    Submit_btn = Button(middle_frame, text='Submit', bg="black", fg="white", width=5, command=ref_input_grid)
...
def ref_input_grid():
    ...
    if r_ref != 0 and c_ref != 0:
        ...
        for i in range(r_ref):
            for j in range(c_ref):
                temp.append(IntVar())
            dic_ref.append(temp)
            temp = []
        Label(right_frame, text="   INPUT MATRIX", fg="white", bg="black", font=('ariel', 15)).grid(row=0, column=1, columnspan=c_ref, padx=10, pady=10)
        for i in range(1, r_ref + 1):
            for j in range(1, c_ref + 1):
                ent = Entry(right_frame, width=4, textvariable=dic_ref[i-1][j-1])
                ent.grid(row=i, column=j + 2, padx=5, pady=5)

        Button(right_frame, text='Submit', font=('ariel', 15), bg="black", fg="white", width=5, command=ref_input_cmd).grid(row=r_ref + 2, column=2, columnspan=c_ref, pady=10)
...
def ref_input_cmd():
    ...
    for i in dic_ref:
        for j in i:
            temp.append(j.get())
        ref_matrix.append(temp)
        temp = []

    ref_result = row_echelon_form(ref_matrix)
    Label(result_frame, text='RESULT', fg="white", bg="black", font=('Ariel', 15)).grid(row=0, column=0, columnspan=len(ref_result[0]))
    for i in range(len(ref_result)):
        for j in range(len(ref_result[0])):
            Label(result_frame, text=ref_result[i][j], padx=6, pady=5, width=5, font=('ariel', 15)).grid(row=i + 1, column=j, padx=5, pady=5)
...
def rank_dimensions_input_row_column():
    frame_destroy()

    global row_rank_dim, col_rank_dim
    Label(middle_frame, text="Enter number of rows of the matrix", bg='black', fg='white', borderwidth=3, relief="groove", font=('ariel', 10), width=35).grid(row=4, column=8, columnspan=5)
    Label(middle_frame, text="Enter number of columns of the matrix", bg='black', fg='white', borderwidth=3, relief="groove", font=('ariel', 10), width=35).grid(row=5, column=8, columnspan=5)

    row_rank_dim = IntVar()
    Entry(middle_frame, width=4, textvariable=row_rank_dim, borderwidth=3, bg='gray64', fg='black', relief="solid").grid(row=4, column=14)
    col_rank_dim = IntVar()
    Entry(middle_frame, width=4, textvariable=col_rank_dim, borderwidth=3, bg='gray64', fg='black', relief="solid").grid(row=5, column=14)

    Label(middle_frame, text="", bg="black").grid(row=10, column=14, padx=80, pady=2, sticky=EW)
    Button(middle_frame, text='Submit', bg="black", fg="white", width=5, command=rank_dimensions_input_grid).grid(row=11, column=14, columnspan=3)
    middle_frame.grid(row=5, rowspan=10, column=2, columnspan=4, padx=200, pady=50)


def rank_dimensions_input_grid():
    r_rank_dim = row_rank_dim.get()
    c_rank_dim = col_rank_dim.get()

    if r_rank_dim != 0 and c_rank_dim != 0:
        global right_frame
        middle_frame.grid_forget()
        right_frame.destroy()
        right_frame = Frame(root, bg='BLACK')
        ...
        temp = []
        dic_rank_dim = []
        for i in range(r_rank_dim):
            for j in range(c_rank_dim):
                temp.append(IntVar())
            dic_rank_dim.append(temp)
            temp = []

        Label(right_frame, text="   INPUT MATRIX", fg="white", bg="black", font=('ariel', 15)).grid(row=0, column=1, columnspan=c_rank_dim, padx=10, pady=10)
        for i in range(1, r_rank_dim + 1):
            for j in range(1, c_rank_dim + 1):
                ent = Entry(right_frame, width=4, textvariable=dic_rank_dim[i-1][j-1])
                ent.grid(row=i, column=j + 2, padx=5, pady=5)

        Button(right_frame, text='Submit', font=('ariel', 15), bg="black", fg="white", width=5, command=rank_dimensions_input_cmd).grid(row=r_rank_dim + 2, column=2, columnspan=c_rank_dim, pady=10)
        right_frame.grid(row=5, rowspan=100, column=50, columnspan=40)
    else:
        messagebox.showerror("Input Error", "The rows and columns can't be zero.")


def rank_dimensions_input_cmd():
    global rank_dim_matrix
    rank_dim_matrix = []
    temp = []
    for i in dic_rank_dim:
        for j in i:
            temp.append(j.get())
        rank_dim_matrix.append(temp)
        temp = []

    rank, dimensions = rank_and_dimensions(rank_dim_matrix)
    result_text = f"Rank: {rank}\nDimensions: {dimensions}"
    Label(result_frame, text=result_text, bg="black", fg="white", font=('Ariel', 15)).grid(row=0, column=0, columnspan=len(rank_dim_matrix[0]))
    result_frame.grid(row=7, column=300, padx=100)
...
def determinant_input_cmd():
    ...
    Label(result_frame,text=determinant_result,font=('ariel',15)).grid(row=50,column=0,columnspan=1,pady=10)
    result_frame.grid(row=7,column=300,padx=100)
...
def transpose_input_cmd():
    ...
    result_frame.grid(row=7,column=300,padx=100)
###########################################################################################################################
def polar_form_input_row_column():
    frame_destroy()
    ...
    col=IntVar()
    Entry(middle_frame,width=4,textvariable=col, borderwidth=3,bg='gray64',fg='black', relief="solid").grid(row=5,column=12)
    Submit_btn=Button(middle_frame,text='Submit',bg="black", fg="white",width=5,command=polarform_input_grid)
    Submit_btn.grid(row=6,column=10,columnspan=3)
    middle_frame.grid(row=5,rowspan=4,column=2,columnspan=4,padx=200,pady=50)
def polarform_input_grid():
    r=row.get()
    c=col.get()
    ...
    if r!=0 and c!=0:
        ...
        Button(right_frame,text='Submit',bg="black", fg="white",font=('ariel', 15),width=5,command=polarform_input_cmd).grid(row=r+1,column=c-1,columnspan=c,pady=10)
        right_frame.grid(row=5,rowspan=100,column=50,columnspan=4,padx=20,pady=50)

    else:
        messagebox.showerror("Input Error","The rows and columns can't be zero.")

def polarform_input_cmd():
    global polarmatrix
    polarmatrix=[]
    temp=[]
    for i in dic:
        for j in i:
            temp.append(j.get())
        polarmatrix.append(temp)
        temp=[]
    polar_result=polar(polarmatrix)
    Label(result_frame, text='RESULT',font=('Ariel',15)).grid(row=0,column=0,columnspan=2,rowspan=10)
    # Label(result_frame,text=polar_result,font=('ariel',15)).grid(row=50,column=0,columnspan=1,pady=10)
    for i in range (len(polar_result)):
        for j in range (len(polar_result[0])):
            Label(result_frame,text=polar_result[i][j],padx=6,pady=5,font=('ariel',15)).grid(row=i+1,column=j,padx=5,pady=5)
    result_frame.grid(row=7,column=300,padx=100)
...
def matrix_cmd():
    ...
    button1=Button(matrix_frame,text="REF",bg="black",fg="white",padx=10,pady=10,command=ref_input_row_column)
    button1.grid(row=5,column=0,padx=10,pady=10,sticky="nsew")
    button1=Button(matrix_frame,text="Rank/dimen",bg="black",fg="white",padx=10,pady=10,command=rank_dimensions_input_row_column())
    ...
    button1=Button(matrix_frame,text="Polar",bg="black",fg="white",padx=10,pady=10,command=polar_form_input_row_column)
    button1.grid(row=12,column=0,padx=10,pady=10,sticky="nsew")
    back=Button(matrix_frame, bg='black',fg='white',text='Home',padx=10,pady=10, width=10,command=back_btn)
    back.grid(row=13,column=0, padx=10, pady=10, sticky="nsew")
...
Button(main_frame, text="Matrix",bg='black',fg='white', font=('Ariel', 18),width=15,command=matrix_cmd).grid(row=7,column=1,columnspan=2,padx=80,pady=2,sticky=EW)
Label(main_frame, text="     ", bg="black").grid(row=7,padx=80,pady=2,sticky=EW)
Button(main_frame, text="Determinant",bg='black',fg='white', font=('Ariel', 18),width=15,command=determinant_cmd).grid(row=9,column=1,columnspan=2,padx=80,pady=2,sticky=EW)
